...
entry = f"| DC-{next_num} | 2026-09-06T22:00:00Z | Introduce pluggable signing backend and mock mode | Created tools/signing_backend.py with local RSA and mock signing. Added mock branches in innocence_chain.py signing functions controlled by AAAC_SIGNING_BACKEND. Existing chain still VERIFIED_OK. Next: add AWS KMS/Azure Key Vault adapters (code only, no credentials). | Mock mode tested; local mode unchanged | Complete |\n"
...
if insert_pos is not None:
    lines.insert(insert_pos, f"| DC-{next_num} | Introduce pluggable signing backend and mock mode | 2026-09-06 |")
    new_index = '\n'.join(lines) + '\n'
else:
...
## Signing Backend & Mock Mode — 2026-09-06 (DC-{next_num})
...
'''
if 'Signing Backend & Mock Mode' not in state_text:
