...
# تحديد بداية دالة serve_dashboard
start_marker = '    def serve_dashboard(self):'
start_idx = text.find(start_marker)
if start_idx == -1:
    print("Could not find serve_dashboard method")
    sys.exit(1)

# تحديد نهاية الدالة: البحث عن السطر التالي الذي يبدأ بـ "    def "
# سيكون بعده مباشرة (أو نهاية الملف إذا لم يوجد)
next_def_idx = text.find('\n    def ', start_idx + len(start_marker))
if next_def_idx == -1:
    end_idx = len(text)
else:
    end_idx = next_def_idx  # نهاية الدالة عند بداية التالية

# الدالة الجديدة
new_method = '''    def serve_dashboard(self):
        """Render an enhanced dashboard with chain summary, agents, and events."""
...
# استبدال الجزء المحدد فقط
new_text = text[:start_idx] + new_method + text[end_idx:]
atomic_write(file, new_text)
print("Successfully replaced serve_dashboard safely")
