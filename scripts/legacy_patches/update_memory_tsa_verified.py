...
entry = f"| DC-{next_num} | 2026-09-06T20:55:00Z | Add tsa_verified field to innocence chain | Modified innocence_chain.py to add verify_rfc3161_token function (basic integrity check using openssl ts -reply) and added tsa_verified boolean to each ring. Default TSA remains http://time.certum.pl via AAAC_TSA_URL. | Verified true in generated ring | Complete |\n"
...
if insert_pos is not None:
    lines.insert(insert_pos, f"| DC-{next_num} | Add tsa_verified field to innocence chain | 2026-09-06 |")
    new_index = '\n'.join(lines) + '\n'
else:
...
## TSA Verification Field — 2026-09-06 (DC-{next_num})
...
'''
if 'TSA Verification Field' not in state_text:
