...
entry = f"| DC-{next_num} | 2026-09-07T15:00:00Z | Add ECDSA signing support | Added ecdsa_local backend to signing_backend.py using prime256v1 keys. Updated innocence_chain.py to delegate signing functions to signing_backend. Regenerated chain with ECDSA and verified. | ECDSA chain VERIFIED_OK | Complete |\n"
...
if insert_pos is not None:
    lines.insert(insert_pos, f"| DC-{next_num} | Add ECDSA signing support | 2026-09-07 |")
    new_index = '\n'.join(lines) + '\n'
else:
...
## ECDSA Support — 2026-09-07 (DC-{next_num})
...
'''
if 'ECDSA Support' not in state_text:
