...
# نبحث عن الدالة الحالية verify_rfc3161_token ونستبدلها بنسخة أقوى
old_func = '''def verify_rfc3161_token(token_b64: str) -> bool:
...
if old_func in text:
    ...
    print("Replaced verify_rfc3161_token with enhanced version")
else:
    print("Old function not found, attempting regex replacement")
    # fallback: استبدال أي دالة بنفس الاسم
    import re
    pattern = r'def verify_rfc3161_token\(token_b64: str\) -> bool:.*?(?=\n\n\ndef |\n\n\Z)'
    text = re.sub(pattern, new_func, text, count=1, flags=re.DOTALL)
    print("Regex replacement done")

# تعديل الاستدعاء في generate_ring ليمرر hash البيانات الأصلي
old_call = "tsa_verified = verify_rfc3161_token(rfc3161_token) if rfc3161_token else False"
new_call = "tsa_verified = verify_rfc3161_token(rfc3161_token, agent_events_hash) if rfc3161_token else False"
if old_call in text:
    text = text.replace(old_call, new_call, 1)
    print("Updated verify call to pass agent_events_hash")
else:
    print("Old call not found, skipping call update")

atomic_write(file, text)
print("Successfully improved TSA verification")
