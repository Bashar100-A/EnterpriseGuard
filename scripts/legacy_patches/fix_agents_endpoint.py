...
# استبدال دالة load_agents بالنسخة الجديدة
old_func = '''def load_agents() -> dict:
    """Return summary of known agents from relational memory (safe subset)."""
    mem = load_json(RELATIONAL_MEMORY_PATH, None)
    if not isinstance(mem, dict):
        return {"agents": [], "note": "relational_memory unavailable"}
    ...
    if isinstance(nodes, list):
        for node in nodes:
            if isinstance(node, dict):
                # نبحث عن أي حقل يحمل معرف الوكيل
                agent_id = node.get("agent_id") or node.get("actor_id") or node.get("event_id")
                if agent_id and not str(agent_id).startswith("EVT-"):
...
if old_func not in text:
    print("Old load_agents function not found")
    sys.exit(1)
